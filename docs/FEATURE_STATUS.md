...
| `reward-reader` | in development | Phase 2C2 minimal single-file write adapter on the current branch | Optional study-to-unlock novel reading module with a default-off feature shell, foundational settings, pure data types, pure store normalization, a detached built-in chapter parser, detached Vault-local source inspection, pure import preparation, pure in-memory store application, detached read-only state/cache storage, and detached canonical-only single-file state/cache writing | Keep read and write storage boundaries detached, then move next into explicit import persistence orchestration only after write ordering, partial-success handling, and read-before-write rules are designed |
...
- Version / phase: Phase 2C2 minimal single-file write adapter on the current branch
- Summary: Reward Reader now includes detached storage read and write boundaries on top of the earlier Phase 1A foundation, Phase 2A parser, Phase 2B1 Vault source inspection, Phase 2B2 pure import preparation, and Phase 2B3 in-memory store application. The current branch keeps the default-off independent feature shell, flat plugin settings fields, dedicated Reward Reader settings tab, pure TypeScript data types, pure store normalization helpers, detached pure chapter parser, detached Vault-local source inspection, detached pure import-payload assembly, detached pure store application, a read-only `DataAdapter` boundary, and now a canonical-only write boundary for `.nestkit/reward-reader/state.json` plus one explicit `.nestkit/reward-reader/indexes/<novel-id>.json` cache at a time. Phase 2C2 can create the needed Reward Reader parent directories and write one target file per successful call, but still does not perform cache/state transactions, write ordering orchestration, partial-success compensation, rollback, import runtime, modal/file picker work, reader UI, exchange engine, sidebar, status bar, listeners, or startup hydration.
- Next step: Keep Phase 2C2 scoped to detached single-file writing, then design Phase 2C3 import persistence orchestration with read-before-write checks, cache/state write ordering, and explicit partial-success handling. Detailed planning now lives in `docs/REWARD_READER_PLAN.md`.
