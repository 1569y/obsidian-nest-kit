...
| `reward-reader` | in development | Phase 2C1 read-only storage adapter on the current branch | Optional study-to-unlock novel reading module with a default-off feature shell, foundational settings, pure data types, pure store normalization, a detached built-in chapter parser, detached Vault-local source inspection, pure import preparation, pure in-memory store application, and a detached read-only state/cache storage boundary | Keep the storage boundary read-only and detached, then move next into explicit write persistence only after missing, invalid, normalized, and future-schema read states are stable |
...
- Version / phase: Phase 2C1 read-only storage adapter on the current branch
- Summary: Reward Reader now includes a detached read-only storage adapter on top of the earlier Phase 1A foundation, Phase 2A parser, Phase 2B1 Vault source inspection, Phase 2B2 pure import preparation, and Phase 2B3 in-memory store application. The current branch keeps the default-off independent feature shell, flat plugin settings fields, dedicated Reward Reader settings tab, pure TypeScript data types, pure store normalization helpers, detached pure chapter parser, detached Vault-local source inspection, detached pure import-payload assembly, detached pure store application, and now also adds a `DataAdapter`-based read boundary for `.nestkit/reward-reader/state.json` and one explicit `.nestkit/reward-reader/indexes/<novel-id>.json` cache at a time. This phase distinguishes missing, ready, normalized, invalid JSON, invalid data, future schema, and read failure states without writing files, creating directories, scanning indexes, or entering startup.
- Next step: Keep Phase 2C1 scoped to read-only state/cache loading, then add explicit write persistence and import runtime only after the read states and future-schema protections are stable. Detailed planning now lives in `docs/REWARD_READER_PLAN.md`.
