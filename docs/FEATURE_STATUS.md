...
| `reward-reader` | in development | Phase 3C3A import-completion pure foundation plus Phase 3C1 study-record command on the current branch | Optional study-to-unlock novel reading module with a default-off feature shell, detached import and study pipelines, real-world external TXT/Markdown import with encoding detection, safer numeric-colon chapter detection, localized preview warnings, prepared Vault-copy reuse, a pure study-minute exchange engine, async write and replay runtimes, a first user-triggered study-record command plus a minimal modal with explicit recovery handling, and two new detached import-completion foundations for historical progress initialization and Markdown render-risk inspection | Keep the new import and study UI explicit and memory-bounded, then wire the import-completion foundations through later modal work without broadening startup work |
...
- Version / phase: Phase 3C3A import-completion pure foundation plus Phase 3C1 study-record command on the current branch
- Summary: Reward Reader now includes the earlier Phase 1A foundation, Phase 2A parser, Phase 2B1 Vault source inspection/cache assembly, Phase 2B2 import preparation, Phase 2B3 in-memory store application, Phase 2C1 read-only adapter, Phase 2C2 single-file writer, Phase 2C3 cache-first persistence orchestration, Phase 2D1 detached import runtime, Phase 2D2 minimal import command and modal, the detached Phase 3A study-minute exchange engine, the detached Phase 3B1 study-exchange persistence runtime, the detached Phase 3B2A replay-inspection layer, the detached Phase 3B2B replay recovery runtime, and the first desktop study-record UI entry above those detached layers. The current Phase 3C3A follow-up keeps the existing import/runtime persistence boundaries unchanged and adds only two detached pure foundations: one module that turns one explicit historical `readThroughChapterIndex` selection into a deterministic initial progress plan with zero study-side effects, and one module that inspects normalized source text with bounded configurable source-ordered samples and five stable non-persistent Markdown risk codes without rewriting source content or schema.
...
- Next step: Keep both the import and study entry points explicit, single-modal, and memory-bounded, then wire the new import-completion foundations through later modal work and decide how later phases should add configurable exchange settings, richer study history surfaces, and reader-facing entry points without broadening startup work. Detailed planning now lives in `docs/REWARD_READER_PLAN.md`.
