...
| `reward-reader` | in development | Phase 2A pure chapter parser on the current branch | Optional study-to-unlock novel reading module with a default-off feature shell, foundational settings, pure data types, pure store normalization, and a detached built-in chapter parser | Keep the parser detached from startup/runtime wiring and move next into Vault file reading plus import assembly only after the pure parser boundary is stable |
...
- Version / phase: Phase 2A pure chapter parser on the current branch
- Summary: Reward Reader now includes its detached pure chapter parser on top of the existing Phase 1A foundation. The current branch keeps the default-off independent feature shell, flat plugin settings fields, dedicated Reward Reader settings tab, pure TypeScript data types, and pure store normalization helpers, and now also adds a built-in chapter parser that reads one in-memory source string and returns chapter offset entries plus lightweight parse metadata. This phase still does not add Vault file reading, import flow, chapter-index persistence, study exchange, reader UI, sidebar UI, status-bar UI, commands, ribbons, or runtime listeners.
- Next step: Keep Phase 2A scoped to the pure parser boundary, validate heading-format coverage and UTF-16 offset behavior, and only then move into Vault file reading, import assembly, and later chapter-index persistence. Detailed planning now lives in `docs/REWARD_READER_PLAN.md`.
