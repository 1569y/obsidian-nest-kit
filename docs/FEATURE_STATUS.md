...
| `reward-reader` | in development | Phase 2B2 pure import assembly on the current branch | Optional study-to-unlock novel reading module with a default-off feature shell, foundational settings, pure data types, pure store normalization, a detached built-in chapter parser, a detached Vault-local source inspection boundary, and a detached pure import-payload assembly layer | Keep the import preparation detached from startup/runtime wiring and move next into explicit import action flow only after the pure payload boundary is stable |
...
- Version / phase: Phase 2B2 pure import assembly on the current branch
- Summary: Reward Reader now includes a detached pure import-preparation layer on top of the earlier Phase 1A, Phase 2A, and Phase 2B1 work. The current branch keeps the default-off independent feature shell, flat plugin settings fields, dedicated Reward Reader settings tab, pure TypeScript data types, pure store normalization helpers, detached pure chapter parser, detached Vault-local source inspection, and now also adds a pure function that validates a normalized existing store plus a successful source inspection result and turns them into a prepared import payload containing one `RewardReaderNovel`, one initial `RewardReaderNovelProgress`, the existing chapter-cache reference, a recommended `primaryNovelIdAfterImport`, and warnings. This phase still does not add import action runtime, import modal, file picker, novel registration into real state, chapter-index persistence, state persistence, study exchange, reader UI, sidebar UI, status-bar UI, commands, ribbons, or runtime listeners.
- Next step: Keep Phase 2B2 scoped to detached payload preparation, validate conflict handling and immutable-input boundaries, and only then move into explicit import action flow plus later state/cache persistence. Detailed planning now lives in `docs/REWARD_READER_PLAN.md`.
