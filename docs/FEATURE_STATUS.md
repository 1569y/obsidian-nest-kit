...
| `reward-reader` | in development | Phase 2D1 detached minimal import runtime flow on the current branch | Optional study-to-unlock novel reading module with a default-off feature shell, foundational settings, pure data types, pure store normalization, detached parser/source inspection/import preparation/store application/read-write storage/persistence orchestration, and now a detached end-to-end import runtime function | Keep the runtime flow detached, then move next into an explicit user-driven import command and modal only after file picking and user-facing partial-success copy are designed |
...
- Version / phase: Phase 2D1 detached minimal import runtime flow on the current branch
- Summary: Reward Reader now includes a detached import runtime function on top of the earlier Phase 1A foundation, Phase 2A parser, Phase 2B1 Vault source inspection/cache assembly, Phase 2B2 import preparation, Phase 2B3 in-memory store application, Phase 2C1 read-only adapter, Phase 2C2 single-file writer, and Phase 2C3 cache-first persistence orchestration. The current branch can validate an explicit import request, read an initial state baseline for preparation, inspect one Vault TXT/Markdown source once, prepare an import payload, and delegate final latest-state persistence to the Phase 2C3 orchestrator. It still does not register an import command, import modal, Vault file picker, reader, exchange engine, sidebar, status bar, listener, rollback/atomic transaction, orphan cache cleanup, or concurrency lock.
- Next step: Keep Phase 2D1 detached and design the user-driven command/modal entry, source picker flow, user-facing partial-success messaging, and retry guidance. Detailed planning now lives in `docs/REWARD_READER_PLAN.md`.
