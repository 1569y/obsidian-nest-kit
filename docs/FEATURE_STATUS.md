...
| `reward-reader` | in development | Phase 3B2B detached study-exchange replay recovery runtime on the current branch | Optional study-to-unlock novel reading module with a default-off feature shell, detached import pipeline, a pure study-minute exchange engine, an async write runtime, a pure replay-inspection layer, and now a detached read-only recovery runtime for write-outcome-unknown evidence checks | Keep recovery read-only and explicit, then layer study-record UI and later reader-facing surfaces without broadening startup work |
...
- Version / phase: Phase 3B2B detached study-exchange replay recovery runtime on the current branch
- Summary: Reward Reader now includes the earlier Phase 1A foundation, Phase 2A parser, Phase 2B1 Vault source inspection/cache assembly, Phase 2B2 import preparation, Phase 2B3 in-memory store application, Phase 2C1 read-only adapter, Phase 2C2 single-file writer, Phase 2C3 cache-first persistence orchestration, Phase 2D1 detached import runtime, Phase 2D2 minimal import command and modal, the detached Phase 3A study-minute exchange engine, the detached Phase 3B1 study-exchange persistence runtime, the detached Phase 3B2A replay-inspection layer, and now a detached Phase 3B2B replay recovery runtime. The new runtime keeps the same explicit original study request, validates and snapshots it before any IO, reads Reward Reader state exactly twice, reads one canonical chapter-index cache exactly once, fixes target cache identity from the first state read, rereads the latest state before replay inspection, aborts if the target novel or cache identity changed, and then delegates evidence classification to the pure Phase 3B2A inspector by passing the latest store plus cache-derived `chapterCount`. It returns only `replay-confirmed`, `not-observed`, or structured blocked or conflict results with stable read statuses and sanitized warnings. It still does not write state or cache files, does not retry Phase 3B1 automatically, does not read source text, does not generate ids or timestamps, does not read current time, does not expose raw store or cache data, and does not add study-record UI, reader UI, sidebar UI, status-bar UI, timers, or startup work.
- Next step: Keep this explicit recovery path read-only and detached, then decide how future study-record UI should surface same-request replay checks without turning `not-observed` into an automatic retry or write-failure claim. Detailed planning now lives in `docs/REWARD_READER_PLAN.md`.
